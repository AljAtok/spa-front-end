...
const SalesBudgetTransactionUploadPage: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await post<UploadResult>(
        "/sales-budget-transactions/upload-excel",
        formData,
        undefined,
        {
  ...
  const handleDownloadTemplate = () => {
    ...
    }/uploads/UPLOAD-TEMPLATE/STORE-BUDGET-UPLOAD-TEMPLATE.xlsx`;
  ...
  return (
    <Box maxWidth={1000} mx="auto" mt={4}>
      <FormHeader
        title="Upload Sales Budget Transactions"
        actionButton={
          <Button
            variant="outlined"
            color="success"
            ...
            sx={{ ml: 2 }}
          >
            Back to Sales Budget Transactions
          </Button>
        }
      />
      <Box mt={4}>
                      ...
                      {result.success.map(
                        (row: UploadSuccessRow, idx: number) => (
                          <TableRow key={idx}>
                            ...
                            <TableCell>{row.bc_name}</TableCell>
                            <TableCell>{row.ifs_code}</TableCell>
                            <TableCell>{row.outlet_name}</TableCell>
                            <TableCell>{row.material_desc}</TableCell>
                            <TableCell>{row.material_group_name}</TableCell>
                            <TableCell>
                              {row.sales_det_qty.toLocaleString()}
                            </TableCell>
                            <TableCell>
...
export default SalesBudgetTransactionUploadPage;
